...
const BASE_URL = 'http://10.10.10.25:3001';
...
export async function POST(
  ...
  if(endPoint==='auth/toen') await checkTokenExpiry();
  ...
  if(res.ok){
    ...
    const respData:APISuccessResponse<any> = await handleTokensInResponse(data) as APISuccessResponse<any>;
    return respData;
  }else{
    throw new Error(res.statusText || "Uh Oh! Something went wrong!");
  }    
...
async function checkTokenExpiry(){
  try {
    ...
    Object.assign(options, await _getCommonOptions());
    options.method = "GET";
    const res = await fetch(`${BASE_URL}/dealers/me`, options);
